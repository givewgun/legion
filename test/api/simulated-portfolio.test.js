...
import { simulatedPortfolioRoutes } from '../../src/api/routes/simulated-portfolio.js';

function build(repo, gunvest) {
  const app = express();
  app.use((req, _res, next) => { req.user = { id: 1 }; next(); });
  app.use('/api/simulated-portfolio', simulatedPortfolioRoutes(repo, gunvest, { horizonDays: 5 }));
...
describe('per-user simulated portfolio', () => {
  beforeEach(() => { gunvestStub.getCandles.mockClear(); });

  it('503s when price data is unavailable', async () => {
    const res = await request(build(repoStub(), null)).get('/api/simulated-portfolio');
    expect(res.status).toBe(503);
  });

  it('simulates only the user watchlist symbols', async () => {
    const repo = repoStub();
    const res = await request(build(repo, gunvestStub)).get('/api/simulated-portfolio');
  ...
  it('serves cached response on second request', async () => {
    ...
    await request(app).get('/api/simulated-portfolio');
    await request(app).get('/api/simulated-portfolio');
  ...
  it('skips a symbol whose candle fetch fails', async () => {
    ...
    const res = await request(build(repoStub(), failing)).get('/api/simulated-portfolio');
    expect(res.status).toBe(200);
  });
