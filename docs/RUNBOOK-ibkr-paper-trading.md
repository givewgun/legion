...
| `skipped (hold)` | The intent's band was `HOLD` (a genuine consensus hold, or risk's `blockBuy` downgrade of a BUY/STRONG_BUY). HOLD is a no-action band — it is never sized or sent to the broker, so an existing position is never liquidated just because the panel went quiet on it. |
| `skipped (superseded)` | A newer pending intent for the same symbol exists. Target weight is absolute, not incremental, so only the newest pending intent per symbol is ever acted on — older same-symbol intents left over from a backlog are stale reads and are skipped rather than processed out of order. |
| `failed` | One of three causes, each recorded in the error text: the broker rejected the order (read it — rejections are not auto-retried on purpose); a resting DAY order was cancelled/expired unfilled; or a `submitted` order's `cOID` was no longer found at the broker on a status check ("order not found at broker (lost after submit)"). A new signal for the same symbol re-sizes against the account's *actual* position, so a failed order self-corrects on the next signal rather than needing a manual resubmit. Note: IBKR's orders endpoint is day-scoped, so a same-day fill that has already cleared the endpoint can also legitimately return not-found — the executor cross-checks the account's current position before concluding the order was lost, and marks it `filled` (with `fill_price` left `null`, since the day-scoped endpoint no longer has it) instead of `failed` when the position is consistent with a fill. |
